<template>
  ...
  >
    <!-- Project Image -->
    <div class="relative h-48 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 overflow-hidden">
      <img 
    ...
    <div class="p-6">
      ...
      <div class="flex flex-wrap gap-2 mb-4">
        <span 
          ...
          :key="tech.id"
          class="flex items-center gap-1.5 px-3 py-1 bg-slate-700/50 text-slate-300 text-xs rounded-lg hover:bg-slate-700 transition-colors"
        >
          <img v-if="tech.icon" :src="tech.icon" :alt="tech.name" class="w-3 h-3" />
          <span v-else class="w-2 h-2 bg-indigo-400 rounded-full"></span>
          {{ tech.name }}
        </span>
        <span 
          v-if="project.technologies?.length > 4"
          class="flex items-center px-3 py-1 bg-slate-700/30 text-slate-400 text-xs rounded-lg"
        >
      ...
      <!-- Project Timeline -->
      <div class="flex items-center justify-between text-xs text-white border-t border-slate-700/50 pt-4">
