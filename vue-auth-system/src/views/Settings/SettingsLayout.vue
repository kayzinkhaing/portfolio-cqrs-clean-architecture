<template>
    <div class="flex min-h-screen bg-gradient-to-br from-gray-50 via-white to-purple-50">
        ...
        <aside class="w-72 bg-white/70 backdrop-blur-xl shadow-2xl flex flex-col border-r border-white/20 animate-slide-in relative">
            <!-- Decorative gradient overlay -->
            <div class="absolute inset-0 bg-gradient-to-br from-purple-500/5 to-pink-500/5 rounded-r-3xl"></div>
            ...
            <div class="relative p-6 border-b border-gray-200/50">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Settings</h2>
                </div>
            </div>
            ...
            <nav class="flex flex-col p-6 space-y-3 flex-1 relative z-10">
                <RouterLink v-for="link in links" :key="link.name" :to="link.to"
                    ...
                    :class="{
                        'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/25': $route.path === link.to,
                        'text-gray-600 hover:bg-white hover:text-purple-700 hover:shadow-purple-500/10': $route.path !== link.to,
                    }">
                    <!-- Active state glow effect -->
                    <div v-if="$route.path === link.to" class="absolute -inset-1 bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl blur opacity-30"></div>
                    ...
                    <component :is="link.icon" 
                        ...
                        :class="{
                            ...
                            'text-purple-600': $route.path !== link.to
                        }"
                    />
                    <span class="relative z-10">{{ link.name }}</span>
                    
                    <!-- Hover glow effect for inactive items -->
                    <div v-if="$route.path !== link.to" class="absolute -inset-1 bg-gradient-to-r from-purple-100 to-pink-100 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                </RouterLink>
            </nav>
            ...
            <div class="p-6 border-t border-gray-200/50 relative z-10">
                <button @click="handleLogout"
                    ...
                    <LogoutIcon class="w-5 h-5 transition-transform duration-300 group-hover:scale-110" />
        ...
        <main class="flex-1 p-8 animate-fade-in">
            ...
            <div class="max-w-5xl mx-auto">
                <div class="bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 min-h-[600px] relative overflow-hidden">
                    <!-- Decorative background elements -->
                    <div class="absolute top-0 right-0 w-72 h-72 bg-gradient-to-br from-purple-400/10 to-pink-400/10 rounded-full blur-3xl"></div>
                    <div class="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-tr from-blue-400/10 to-purple-400/10 rounded-full blur-3xl"></div>
...
// Lazy-load icons for modern feel
...
const links = [
  { name: 'Information', to: '/settings/info', icon: InformationCircleIcon },
  { name: 'Two-Factor Authentication', to: '/settings/2fa', icon: ShieldCheckIcon },
  { name: 'Your Blogs', to: '/settings/blogs', icon: DocumentTextIcon },
]
